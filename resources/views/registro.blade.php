...
<section class="container-fluid">
    ...
    <div class="container-fluid mt-5">
        <div class="progress-steps flex-wrap">
            ...
            <div class="step" data-step="3">Datos de Participación</div>
            <div class="step" data-step="4">Logística</div>
            <div class="step" data-step="5">Idioma Preferido</div>
            <div class="step" data-step="6">Información de Facturación</div>
            <div class="step" data-step="7">Consentimientos</div>
        </div>
        <form action="{{ route('registro.complete') }}" method="post" id="multiStepForm">
            @csrf
            ...
            <div class="form-step container" id="step-2">
                <div class="row">
                    ...
                    <div class="col-12 col-md-4 mt-3">
                        <label for="faculty" class="form-label">Departamento/Facultad</label>
                        <input type="text" class="form-control" id="faculty" name="faculty" required placeholder="Ej. Sistemas Computacionales" value="{{ old('faculty', $formData['faculty'] ?? '') }}">
                    </div>
                    <div class="col-12 col-md-4 mt-3">
            ...
            <div class="form-step container" id="step-3">
                <div class="row">
                    <div class="col-12 mt-3">
                        <label for="participation">Tipo de Participación</label><br>
                        <div class="d-flex justify-content-between">
                            <div>
                                <input type="radio" name="participation" id="participation_1" value="1" {{ old('participation', session('form_data.participation')) == '1' ? 'checked' : '' }} required>&nbsp;
                                <span>Asistente</span>
                            </div>
                            <div>
                                <input type="radio" name="participation" id="participation_2" value="2" {{ old('participation', session('form_data.participation')) == '2' ? 'checked' : '' }} required>&nbsp;
                                <span>Ponente oral</span>
                            </div>
                            <div>
                                <input type="radio" name="participation" id="participation_3" value="3" {{ old('participation', session('form_data.participation')) == '3' ? 'checked' : '' }} required>&nbsp;
                                <span>Presentación de cartel</span>
                            </div>
                            <div>
                                <input type="radio" name="participation" id="participation_4" value="4" {{ old('participation', session('form_data.participation')) == '4' ? 'checked' : '' }} required>&nbsp;
                                <span>Taller y Sesión paralela</span>
                            </div>
                        </div>
                    </div>
                    <div class="col-12 col-md-6 mt-3">
                        <label for="presentation">Título de la ponencia(si aplica)</label>
                        <input type="text" class="form-control" id="presentation" name="presentation" placeholder="Ej. Cuidado del Agua" value="{{ old('presentation', $formData['presentation'] ?? '') }}">
                    </div>
                    <div class="col-12 col-md-6 mt-3">
                    ...
                    <div class="col-12 mt-3">
                        ...
                        <input type="radio" name="proof" id="proof" value="1" {{ old('proof', session('form_data.proof')) == '1' ? 'checked' : '' }} required>&nbsp;
                        <span>Sí</span>
                        <input type="radio" name="proof" id="proof" value="0" {{ old('proof', session('form_data.proof')) == '0' ? 'checked' : '' }} required>&nbsp;
                        <span>No</span>
                    </div>
                    <div class="btn-container">
                        <button type="button" class="btn btn-primary btn-prev">Anterior</button>
            ...
            <div class="form-step container" id="step-5">
                <div class="row">
                    <div class="col-12 mt-3">
                        ...
                        <div class="d-flex justify-content-between">
                            <div>
                                <input type="radio" name="language" id="language_es" value="Español"
                                        {{ old('language', session('form_data.language')) == 'Español' ? 'checked' : '' }} required>&nbsp;
                                <span>Español</span>
                            </div>
                            <div>
                                <input type="radio" name="language" id="language_en" value="Inglés"
                                {{ old('language', session('form_data.language')) == 'Inglés' ? 'checked' : '' }} required>&nbsp;
                                <span>Inglés</span>
                            </div>
                            <div>
                                <input type="radio" name="language" id="language_other" value="Otro"
                                {{ (!in_array(old('language', session('form_data.language')), ['Español', 'Inglés'])) ? 'checked' : '' }}>&nbsp;
                                <span>Otro</span>
                                <input type="text" name="other_language" id="other_language_input"
                                class="form-control mt-1 {{ (!in_array(old('language', session('form_data.language')), ['Español', 'Inglés'])) ? '' : 'd-none' }}" placeholder="Especificar otro idioma" value="{{ (!in_array(old('language', session('form_data.language')), ['Español', 'Inglés'])) ? old('other_language', session('form_data.other_language')) : '' }}">
                            </div>
                        </div>
                    </div>
                    <div class="btn-container">
...
<script>
    ...
    async function handleNextStep(currentStep) {
        ...
        try {
            ...
            const response = await fetch(`/registro/step/${currentStep}`, {
                method: 'POST',
                headers: {
    ...
    document.getElementById('btn-save-final').addEventListener('click', async function() {
        ...
        try {
            ...
            const validationResponse = await fetch('/registro/step/7', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
