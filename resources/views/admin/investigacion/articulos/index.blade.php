...
@section('content_header')
    <h1 class="m-0 text-dark fw-bolder">Articulos</h1>
@stop
        ...
        <table class="table table-striped" id="tecin">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($a->link != null)
                        <a class="btn btn-primary" href="{{$a->link}}" target="_blank">
                            <i class="fa fa-eye"></i>
                        </a>
                        @endif
                    </td>
                    <td class="text-center">{{$a->tecnologico}}</td>
                    <td class="text-center">
                        <button class="btn btn-warning" data-toggle="modal" data-target="#editR{{$a->id}}">
                            <i class="fa fa-pen" aria-hidden="true"></i>
                        </button>
                        <button class="btn btn-danger destroy" data-toggle="modal" data-target="#deleteR{{$a->id}}">
                            <i class="fa fa-trash" aria-hidden="true"></i>
                        </button>
                    </td>
                </tr>
                @include('admin.investigacion.articulos.modals.edit')
                @include('admin.investigacion.articulos.modals.delete')
                @endforeach
            </tbody>
        </table>
        </div>
...
@include('admin.investigacion.articulos.modals.create')
@stop
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            window.location.href = "{{ route('articulos') }}";
                        });
                    } else {
                        Swal.fire({
                            type: 'error',
