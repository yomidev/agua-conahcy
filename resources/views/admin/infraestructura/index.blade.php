...
@section('content_header')
    <h1 class="m-0 text-dark fw-bolder">Infraestructura por Tecnológico</h1>
@stop

@section('content')
<div class="card">
    <div class="card-header">
        <button type="button" class="btn btn-success float-right glyphicon" data-toggle="modal" data-target="#addEquipment">
        ...
        <table class="table table-striped" id="tecin">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">{{$e->nombre_equipo}}</td>
                    <td class="text-center">{{$e->tecnologico}}</td>
                    <td class="text-center">
                        <button class="btn btn-success" data-toggle="modal" data-target="#ViewEquipment{{$e->id}}">
                            <i class="fa fa-eye" aria-hidden="true"></i> Visualizar
                        </button>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#editEquipment{{$e->id}}">
                            <i class="fa fa-pen" aria-hidden="true"></i> Editar
                        </button>
                        <button class="btn btn-danger destroy" data-toggle="modal" data-target="#deleteEquiment{{$e->id}}">
                            <i class="fa fa-trash" aria-hidden="true"></i> Eliminar
                        </button>
                    </td>
                </tr>
                @include('admin.infraestructura.modals.view')
                @include('admin.infraestructura.modals.edit')
                @include('admin.infraestructura.modals.delete')
                @endforeach
            </tbody>
        </table>
        </div>
...
@include('admin.infraestructura.modals.create')
@stop
...
<script>
    ...
    $(document).ready( function () {
        ...
        $('.create_equipment').on('submit', function(e) {
            e.preventDefault();
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            window.location.href = "{{ route('equipment') }}";
                        });
                    } else {
                        Swal.fire({
                            type: 'error',
        ...
        $('.update_equipment').on('submit', function(e) {
            e.preventDefault();
            var formData = new FormData(this);
            $.ajax({
                url: $(this).attr('action'),
