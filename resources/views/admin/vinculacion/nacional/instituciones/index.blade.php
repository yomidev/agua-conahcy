...
@section('content_header')
    <h1 class="m-0 text-dark fw-bolder">Instituciones Nacionales</h1>
@stop
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @if($i->logo != null)
                        <button class="btn btn-success" data-toggle="modal" data-target="#ViewR{{$i->id}}">
                            <i class="fa fa-image" aria-hidden="true"></i>
                        </button>
                        @endif
                        <button class="btn btn-warning" data-toggle="modal" data-target="#editR{{$i->id}}">
                            <i class="fa fa-pen" aria-hidden="true"></i>
                        </button>
                        <button class="btn btn-danger destroy" data-toggle="modal" data-target="#deleteR{{$i->id}}">
                            <i class="fa fa-trash" aria-hidden="true"></i>
                        </button>
                    </td>
                </tr>
                @include('admin.vinculacion.nacional.instituciones.modals.view')
                @include('admin.vinculacion.nacional.instituciones.modals.edit')
                @include('admin.vinculacion.nacional.instituciones.modals.delete')
                @endforeach
            </tbody>
        </table>
        </div>
...
@include('admin.vinculacion.nacional.instituciones.modals.create')
@stop
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            window.location.href = "{{ route('instituciones') }}";
                        });
                    } else {
                        Swal.fire({
                            type: 'error',
