...
@section('content_header')
    <h1 class="m-0 text-dark fw-bolder">Oferta Educativa por Tecnológico</h1>
@stop

@section('content')
<div class="card">
    <div class="card-header">
        <button type="button" class="btn btn-success float-right glyphicon" data-toggle="modal" data-target="#addPosgrado">
        ...
        <div class="container-fluid table-responsive">
            <table class="table table-striped" id="tecin">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-center">{{$p->id_program}}</td>
                        ...
                        <td class="text-center">
                            <button class="btn btn-warning" data-toggle="modal" data-target="#editProgram{{$p->id_program}}">
                                <i class="fa fa-pen" aria-hidden="true"></i>
                            </button>
                            <button class="btn btn-danger destroy" data-toggle="modal" data-target="#deleteProgram{{$p->id_program}}">
                                <i class="fa fa-trash" aria-hidden="true"></i>
                            </button>
                        </td>
                    </tr>
                    @include('admin.posgrados.modals.edit')
                    @include('admin.posgrados.modals.delete')
                    @endforeach
                </tbody>
            </table>
        </div>
...
@include('admin.posgrados.modals.create')
@stop
...
<script>
    ...
    $(document).ready( function () {
        ...
        $('.create_posgrado').on('submit', function(e) {
            e.preventDefault();
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            window.location.href = "{{ route('posgrados') }}";
                        });
                    } else {
                        Swal.fire({
                            type: 'error',
